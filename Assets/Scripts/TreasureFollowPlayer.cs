...
public class TreasureFollowPlayer : MonoBehaviour
{
    ...
    public int magnetPowerUpCounter = 0;
    ...
    void OnTriggerEnter2D(Collider2D col)
    {
        ...
        {
            magnetPowerUpCounter++;
            if(magnetPowerUpCounter == 31)
            {
                magnetPowerUpEnabled = false;
                magnetPowerUpCounter = 0;
            }
        }
    ...
    void Update()
    {
        if (magnetPowerUpEnabled)
        {
            ...
            {
                treasure.transform.position = Vector2.MoveTowards(treasure.transform.position, transform.position, 1200.0f * Time.deltaTime);
            }
        }
    }
}
